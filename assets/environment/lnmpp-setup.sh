...
devUserPower(){
  ...
  usermod -G nginx,php-fpm,postgres,redis,mysql $devUserName
}
...
modFilePower(){
  ...
  echo_yellow "=================================================================="
  echo_green "站点根目录权限"
  echo_green "开发环境使用emad用户（nginx/php-fpm 需加入 emad用户组）"
  echo_green "部署环境通常是www用户（nginx/php-fpm 需加入 www用户组）"
  chmod 750 /www
